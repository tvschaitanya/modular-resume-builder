...
- Only include experience, projects, and skills that are directly relevant to the job description (JD). Remove anything that does not align with what the role actually requires.
- Focus on accomplishments and responsibilities that match the JD's key qualifications and keywords.
...
### Step 3 — Keyword Gap Analysis (mandatory before rewriting)
...
### Step 4 — Build or Update resume.json
...
User says: _Here's the JD (paste). Here's my current resume.json (paste)._
...
2. For misworded items, reword to match JD language honestly. For example, "built event pipelines with SQS" can be reframed to align with a JD that mentions event-driven architecture because the experience is real and only the framing changes
3. For missing items, do not add them silently. Tell the user clearly instead:
   - "Kafka is listed as required and is not in your background. If you have any adjacent experience with message queues or event streaming, let me know and I can frame it honestly."
   - "Go is listed as required and has no overlap in your resume. Worth knowing before applying since some companies are flexible and others are not."
4. Never bridge a gap by inventing experience. Surface it, give the user options, and let them decide
5. Output a gap summary covering what was changed, what could not be bridged, and then the full updated resume.json

---

## Writing Style Guide (Best Practices)

When rewriting bullets, follow these principles for the strongest output:

- Start every bullet with a strong action verb like "Built," "Designed," "Optimized," or "Engineered." No "I" statements and no narrative backstory.
- Each bullet should say what you did, how you did it, and ideally what it achieved, all in one clean sentence.
- Mention technologies within the bullet itself rather than tacking them on as a separate trailing list.
- The bullet is not the place to describe what a system or company does. Focus purely on your contribution.
- Match the tone to the seniority level. Senior roles call for words like "led," "owned," or "architected." Mid-level roles fit "built," "developed," or "improved."
- Reframing real experience to match JD language is fair and encouraged. Adding experience that does not exist is not.
- Keep the summary to two or three lines, written in third-person implied style, targeting the specific role and seniority level in the JD.
